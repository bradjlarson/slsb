...
Template.explorer.is_admin = function() {
...
Template.explorer_admin.rendered = function() {
...
Template.explorer_admin.events = {
...
//Database Explorer
Template.explorer_admin.databases = function() {
...
Template.explorer_admin.tables = function() {
...
//Database Explorer Events:
Template.explorer_analyst.events = {
	'click .db_results' : function(event) {
		$('.db_results').removeClass('info'); 
		var selected = event.currentTarget;
		$(selected).addClass('info');
...
//Database Explorer
Template.explorer_analyst.databases = function() {
	var search_query = Session.get("db_searched");
	var mongo_query = '.*'+search_query+'.*';
	if (search_query)
	{
...
Template.explorer_analyst.tables = function() {
	var search_query = Session.get("table_searched");
	var mongo_query = '.*'+search_query+'.*';
	if (search_query)
	{
...
Template.explorer_analyst.columns = function() {
	return columns.find({database_name : Session.get("db_selected"), table_name : Session.get("table_selected")}, {sort : {column_name : 1}});
}

Template.explorer_analyst.selected_table = function() {
	return tables.find({database_name : Session.get("db_selected"), table_name : Session.get("table_selected")});	
}

Template.explorer_analyst.db_selected = function() {
	return Session.get("db_selected") ? true : false;
}

Template.explorer_analyst.table_selected = function() {
	return Session.get("table_selected") ? true : false;
}
